...
import (
	...
	"github.com/Wishmob/bookingsApp/internal/repository/dbrepo"
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	message := fmt.Sprintf(`
		...
		this is to confirm your reservation of our room %s from %s to %s.<br><br>

		Yours<br>
		Boss from se hotel <3
	`, reservation.FirstName, reservation.LastName, reservation.Room.RoomName, reservation.StartDate.Format("2006-01-02"), reservation.EndDate.Format("2006-01-02"))

	confirmationMail := models.MailData{
		From:     "boss@hotel.com",
	...
	m.App.MailChan <- confirmationMail

	//send confirmation mail to owner
	message = fmt.Sprintf(`
		Dear Boss from se hotel,<br><br>

		a new reservation of room %s from %s to %s has been made.<br><br>

		Have a nice Day!
	`, reservation.Room.RoomName, reservation.StartDate.Format("2006-01-02"), reservation.EndDate.Format("2006-01-02"))

	infoMail := models.MailData{
		From:    "info@hotel.com",
		To:      "boss@hotel.com",
		Subject: "New Reservation",
		Content: message,
	}
	m.App.MailChan <- infoMail
...
func (m *Repository) ChooseRoom(w http.ResponseWriter, r *http.Request) {
	//switch these 6 lines for the lines below to test easier
	...
	// use this to test it more easily
...
//PostShowLogin handles login in the user
...
// AdminAllReservations shows all new reservations in admin tool
...
// AdminAllReservations shows single reservation in admin tool
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, room := range rooms {
		...
		// remove.
		currentBlockMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", room.ID)).(map[string]int)
		for name, value := range currentBlockMap {
			// ok will be false if the value is not in the map
			if restrictionID, ok := currentBlockMap[name]; ok {
				// only pay attention to restriction id's > 0, and that are not in the form post
